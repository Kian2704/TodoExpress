...
export default function NewListContainer({ fetchHomeLists }) {
  ...
  let submitForm = async (event) => {
    ...
    if (!listName || tasks.length <= 0) return;
    let body = {
      listName: listName,
      tasks: tasks,
    };
    ...
    {
      setErrorMessages([...errorMessages,'Internal Server Error, if this error persists please contact an Administrator.']);
    }
  };
    ...
    <div className="container listContainer mt-5">
      <div className="container col col-10 col-md-9 rounded-1 ">
        <form onSubmit={submitForm}>
          ...
          {tasks && tasks.map((task) => <NewListTask taskName={task} />)}
